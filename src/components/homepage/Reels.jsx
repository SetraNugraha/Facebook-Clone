  ...
  return (
    <>
      <li className={`h-full w-full flex justify-center items-center ${activeMenu === title ? ' border-b-[3px] border-blue-500' : 'border-b-[2px] border-slate-400'} `}>
        <button onClick={() => handleMenuClick(title)} className="flex gap-2 justify-center items-center">
  ...
  return (
    <>
      <a href="#" className="h-full flex flex-col justify-between">
        ...
        <img src={profileImage} alt="" className="rounded-t-xl h-[75%]" />
        {/* Circle Plus */}
      ...
      <li className="relative">
        <a href="#">
          ...
          <div className="absolute inset-0 flex items-start justify-start py-3 px-3 ">
            <a href="#">
              <img src={userProfileImage} alt="" width={'38px'} height={'38px'} className="rounded-full border-4 border-blue-600" />
            </a>
          </div>
          {/* Background Reels Image */}
          <img src={userReelsImage} alt="" className="object-cover w-full h-full rounded-xl" />
          {/* Display Name User */}
          <div className="absolute inset-0 flex items-end justify-start px-5 py-3">
            <p className="text-white text-center font-bold align-bottom">{username}</p>
          </div>
        </a>
      </li>
    </>
      ...
      <div className="bg-white h-[280px] rounded-md drop-shadow-sm z-0">
        ...
        <div className="h-[20%] w-[96%] mx-auto flex justify-center items-center">
          ...
          <ul className="mt-3 w-full h-[70%] flex justify-around items-center">
            <HeaderReels title={'Cerita'} Icon={FaBookOpen} activeMenu={activeMenu} handleMenuClick={handleMenuClick} />
            <HeaderReels title={'Reels'} Icon={CgClapperBoard} activeMenu={activeMenu} handleMenuClick={handleMenuClick} />
          </ul>
        </div>
        {/* End Header */}
